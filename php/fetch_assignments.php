...
try{
    ...
    foreach ($assignments as $assignment){
        try{
            $id = $assignment['id'];
            //echo $id;
            $sql="SELECT student_name,assignment_dir FROM student_submission WHERE assignment_id='$id'";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            for($i=0; $i<count($submissions); $i++){
                $submissions[$i]['assignment_dir'] = 'file://'.getcwd().'/'.$submissions[$i]['assignment_dir'];
            }
