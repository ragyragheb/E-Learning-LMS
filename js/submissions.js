...
export const getAssignments = () => AJAX_REQUEST('GET','fetch_assignments')
            .then(
                data => {
                    var assignments = JSON.parse(data).assignments;
                    for(var i=0; i<assignments.length; i++){
                        
                        //makeing an assignment card, and making a p (assignment name)
                        ...
                        for(var j = 0; j<assignments[i].submissions.length;j++){
                            ...
                            link.href= assignments[i].submissions[j].assignment_dir;
